...
class HDF5Reader(abc.ABC):
    ...
    def __init__(self, filename: str, verbosity: int = 0, skip_paths: str = "") -> None:
        """
        ...
        Parameters:
            ...
            skip_paths (str, optional): Path to the file containing the fragment paths to skip.
        Returns nothing.
        """
        # Generic loading
    ...
    def read_all_fragments(self) -> None:
        """ Filter paths of interest first """
        _ = self._filter_fragment_paths()
        """ Read all fragments. """
        for fragment_path in self._fragment_paths:
            _ = self.read_fragment(fragment_path)
            # print(f"INFO: Read fragment {fragment_path}","\n")

...
class TPReader(HDF5Reader):
    ...
    def __init__(self, filename: str, trg_record_id: int, verbosity: int = 0) -> None:
        """
        Loads a given HDF5 file.

        Parameters:
            filename (str): HDF5 file to open.
            trg_record_id (int): Trigger Record ID to load.
            verbosity (int): Verbose level. 0: Only errors. 1: Warnings. 2: All.

        Returns nothing.
        """
        self.trg_record_id = trg_record_id
        super().__init__(filename, verbosity)
        self.tp_data = np.array([], dtype=self.tp_dt)
        return None

    def _filter_fragment_paths(self) -> None:
        """ Filter the fragment paths for TPs. """
        fragment_paths = []
        
        # Convert trg_record_id to string for comparison
        trg_record_id_str = str(self.trg_record_id)

        # TP fragment paths contain their name in the path.
        for path in self._fragment_paths:
            if trg_record_id_str in path:
